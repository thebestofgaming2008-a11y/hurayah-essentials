...
export const createProductMediaUpload = action({
  ...
  handler: async (ctx, args) => {
    ...
    if (!ALLOWED_TYPES.has(args.contentType)) throw new Error("Only product images and MP4/WebM videos can be uploaded.");
    ...
    const siteUrl = (process.env.PUBLIC_SITE_URL || "https://hurayrahessentials.com").replace(/\/+$/, "");
    return {
