...
const resetEmailProvider = Email({
  id: "password-reset",
  name: "Password reset",
  from: process.env.AUTH_EMAIL_FROM ?? "Hurayrah Essentials <no-reply@hurayrahessentials.com>",
  maxAge: 30 * 60,
  ...
  async sendVerificationRequest({ identifier, token }) {
    ...
    const from = process.env.AUTH_EMAIL_FROM ?? "Hurayrah Essentials <no-reply@hurayrahessentials.com>";
    if (!apiKey) {
      throw new Error("Password reset email is not configured. Add RESEND_API_KEY in Convex environment variables.");
    }
    const response = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from,
        to: identifier,
        subject: "Reset your Hurayrah Essentials password",
        text: `Assalamu alaikum,\n\nUse this code to reset your Hurayrah Essentials password:\n\n${token}\n\nThis code expires in 30 minutes. If you did not request this, you can ignore this email.`,
        html: `
          <div style="font-family:Arial,sans-serif;line-height:1.6;color:#06133a">
            <h2 style="margin:0 0 12px">Reset your Hurayrah Essentials password</h2>
